...
{{~#devicesoftype E.S 'Container'}}
	{{~#each .}}
		{{~#if CustomName}}
			...
			{{~#test Id in '4350,4375,4374,4357,4324,4325,4401,4340,4338,4339,4331,4328,4329,4314,4307,4308,4315,4303,4304,4311,4360,4361,4363,4364,4306,4310,4312,4343,4313,4305,4330,4309,4334,4337,4366,4367,4346,5097,7344,7345,'}}
{{~#move this @root.E.S 'Box-Alles-Input' 1000 }}
...
{{~#move this @root.E.S 'Box-Alles-Input' 200}}
 <pos=1>{{datetime 'HH:mm:ss'}}</pos> <pos=75>| <color=green>Refill</color></pos><pos=125>| {{Count}}</pos><pos=180>| {{i18n Id}} </pos><pos=460>| {{Source}} <color=green>=</color> {{Destination}}</pos>
{{/move}}
			{{/test}}
			{{~#test Id 'in' @Ids.Ingot}}
{{~#move this @root.E.S 'Box-Alles-Input' 2000}}
 <pos=1>{{datetime 'HH:mm:ss'}}</pos> <pos=75>| <color=green>Refill</color></pos><pos=125>| {{Count}}</pos><pos=180>| {{i18n Id}} </pos><pos=460>| {{Source}} <color=green>=</color> {{Destination}}</pos>
{{/move}}
			{{/test}}
