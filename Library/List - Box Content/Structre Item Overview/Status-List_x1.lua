...
<color={{~#test Count geq @root.Data.limitGood}}green>{{~else}}{{~#test Count le @root.Data.limitMedium}}{{~#test Count le @root.Data.limitLow}}red>{{else}}orange>{{/test}}{{~else}}yellow>{{/test}}{{/test}}◉ {{i18n Key}}</pos> <pos=140>{{format Count '{0,6:N0}'}}</pos></color>
{{/test}}
{{/sortedeach}}
