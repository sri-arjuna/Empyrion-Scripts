{{~#devices E.S. '*Box-P*'}}{{~#each .}}{{~set 'CN' CustomName}}{{~set 'Vol' 0}}{{~#split CustomName '-'}}{{~set 'DN' .1}}{{~/split}}{{~#math @root.data.Vol '+' (calc ./Device.VolumeCapacity '/' 1000) }}{{~set 'Vol' .}}{{~/math}}{{~#devices @root.E.S (concat @root.Data.DN '_Display')}}{{~#each .}}{{~settextblock .}}	<align=center><line-height=0>
	</line-height>
