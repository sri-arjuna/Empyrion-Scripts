-- ----------------------------
-- Display cool layout on Harvest Controller, combo with 2 parts: 2*LCD Script + 2*LCD output
-- Requires:
...
-- Output:
--          * Box Name: [996633]Box-<NAME>		('Box-Harvest Loot')
--          * Projector: 'Harvest Loot_dd'
-- * Optionals:
...
-- -------------------------------
-- Requires:
--          * LCD:                        Script:HarvestBar
--          * Projector:                  <Box Name>_DB		('Harvest Loot_DB')
--          * Projector - "z Rotation":   -90
--          * Projector - Offset:         Change so it is displayed on the other LCD empty vertial space.
-- by Sephrajin
-- -----------------------------------------
{{~#devicesoftype E.S. 'HarvestCntr'}}{{~#each .}}{{~set 'CN' CustomName}}{{~set 'Vol' ./Device.VolumeCapacity}}{{~set 'Load' 0}}
