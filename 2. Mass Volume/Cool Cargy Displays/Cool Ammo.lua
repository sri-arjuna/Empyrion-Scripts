...
--  Output:	
--          * Name: [FF3333]Box-<NAME>  ('Box-Ammo')
--          * Projector: 'Ammo_dd'
-- * Optionals:
--             Change font color to match its purpose
--  Original design by sulusdacor, modified by Sephrajin
-- -----------------------------------------------
{{~#devicesoftype E.S. 'AmmoCntr'}}{{~#each .}}{{~set 'CN' CustomName}}{{~set 'Vol' 0}}{{~#split CustomName '-'}}{{~set 'DN' .1}}{{~/split}}{{~#math @root.data.Vol '+' (calc ./Device.VolumeCapacity '/' 1000) }}{{~set 'Vol' .}}{{~/math}}{{~#devices @root.E.S (concat @root.Data.DN '_dd')}}{{~#each .}}{{~settextblock .}}	<align=center><line-height=0>
	</line-height>
...
-- Requires:
--          * LCD:                        Script:AmmoBar
--          * Projector:                  <Box Name>_DB		('Ammo_DB')
--          * Projector - "z Rotation":   -90
--          * Projector - Offset:         Change so it is displayed on the other LCD empty vertial space.
-- by Sephrajin
-- -----------------------------------------
{{~#devicesoftype E.S. 'AmmoCntr'}}{{~#each .}}{{~set 'CN' CustomName}}{{~set 'Vol' ./Device.VolumeCapacity}}{{~set 'Load' 0}}
...
{{~#devices @root.E.S (concat @root.Data.DN '_db')}}{{~#each .}}{{~settextblock .}}
<size=-2> </size>
