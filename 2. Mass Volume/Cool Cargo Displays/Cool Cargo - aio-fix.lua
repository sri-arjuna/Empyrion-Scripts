-- ----------------------------
-- Display cool layout on Ammo Controller, combo with 2 parts: 2*LCD Script + 2*LCD output
-- Requires:
...
--			* Name:  Script:HarvestDisplay
--  Output:	
--          * Name: [FF3333]Box-<NAME>  ('Box-Ammo Left')
--          * Projector: 'Ammo Left_Display'
-- * Optionals:
--             Change font color to match its purpose
--  Original design by sulusdacor, modified by Sephrajin
-- -----------------------------------------------
{{~#devices E.S. '*Box-*'}}{{~#each .}}{{~set 'CN' CustomName}}{{~set 'Vol' 0}}{{~#split CustomName '-'}}{{~set 'DN' .1}}{{~/split}}{{~#math @root.data.Vol '+' (calc ./Device.VolumeCapacity '/' 1000) }}{{~set 'Vol' .}}{{~/math}}{{~#devices @root.E.S (concat @root.Data.DN '_Display')}}{{~#each .}}{{~settextblock .}}	<align=center><line-height=0>
	</line-height>
...
-- Requires:
--          * LCD:                        Script:CargoBar
--          * Projector:                  <Box Name>_Bar		('Ammo Left_Bar')
--          * Projector - "z Rotation":   -90
--          * Projector - Offset:         Change so it is displayed on the other LCD empty vertial space.
-- Notes:
--          Some items cannot be properly identified for volume.
--          Yes they are shown properly ingame, but the script seems not to recognize them.
-- by Sephrajin
-- -----------------------------------------
{{~#devices E.S. '*Box-*'}}{{~#each .}}{{~set 'CN' CustomName}}{{~set 'Vol' ./Device.VolumeCapacity}}{{~set 'Cur' 0}}
...
DN:{{@root.data.DN}}/Vol:{{@root.data.Vol}}/t:{{@root.data.T}}/p:{{@root.data.P}}
