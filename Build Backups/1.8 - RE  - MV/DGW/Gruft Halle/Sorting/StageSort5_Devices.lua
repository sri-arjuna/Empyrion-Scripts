{{#test (calc @root.CycleCounter '%' 13) eq 0}}
	...
	{{~#devices @root.E.S 'dl-*'}}
		{{~#each .}} 
			{{~#split CustomName '-'}}
			{{~#test Length eq '3'}}
			{{~#test ../../../../Id 'in' (concat @Ids.DeviceL '266,414,720,1131,1315,1687,1809,1811,2125,2147,2149,2150,2209,2230,2231,2235,2337,2338,2339,2340,2341,2029,2030,2031,2198,2205,2222,2690,2694,2696,2698,2699,2717,2718,2720,2722,2724,2726,2728,2730,2732,2734,2736,2738,2740,2742,2755,2955') }} 
...
{{/move}}
	...
	{{~#devices @root.E.S 'ds-*'}}
		{{~#each .}} 
			{{~#split CustomName '-'}}
			{{~#test Length eq '3'}}
			{{~#test ../../../../Id 'in' (concat @Ids.DeviceS '589,730,712,715,721,1020,1093,1118,1119,1230,1642,1684,1685,1686,1688,1693,1712,1800,1810,2026,2027,2028,2124,2126,2148,2151,2152,2203,2204,2232,2233,2336,2119,2173,2259,2263,2271,2411,2413,2689,2695,2697') }}
{{~#move ../../../../../. @root.E.S ../../../CustomName}}
 <pos=1>{{datetime 'HH:mm:ss'}}</pos> <pos=75>| <color=yellow>Sort</color></pos><pos=125>| {{Count}}</pos><pos=180>| {{i18n Id}} </pos><pos=460>| {{Source}} <color=green>❱</color> {{Destination}}</pos>
{{/move}}
			{{/test}}
